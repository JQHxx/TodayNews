package com.news.today.tools.refresh;
...
/**
 * Created by anson on 2017/8/25. 这是一个自定义的下拉刷新、上拉加载控件 必须自定义 不提供默认的效果
 */

public class ToolsSelfRefreshView extends LinearLayout {

    private static final String TAG = ToolsSelfRefreshView.class.getSimpleName();
    // 刷新时状态
     ...
     * @param context
     */
    public ToolsSelfRefreshView(Context context) {
        super(context);
        init(context);
    }
     ...
     * @param attrs
     */
    public ToolsSelfRefreshView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
     ...
     * @param defStyleAttr
     */
    public ToolsSelfRefreshView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    /**
     * 这是一个自定义的下拉刷新、上拉加载控件 必须自定义 不提供默认的效果
